$(function(){
  $(".comment").on("click", ".reply_form_trigger", function(){
    ...
    var input = $("<textarea id='input_comment_comment' type='text'>");
    var commentableTypeField = $("<input type='hidden' id='commentable_type' value='Comment'>");
    var commentableIdField = $("<input type='hidden' id='commentable_id' value='" + commentId + "'>");
  ...
  $(".comment").on("submit", $("#submit_comment_comment"), function(){
    ...
    $.ajax({
      url: location.pathname + "/comment_comments",
      type: "POST",
